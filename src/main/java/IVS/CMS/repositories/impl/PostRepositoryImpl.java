...
public class PostRepositoryImpl implements PostRepository {
    ...
    public Post save(Post post) {
        if (post.getPostId() == null || post.getPostId() == 0) {
            String sql = """
                    INSERT INTO posts (title, slug, summary, content, status, category_id, created_at, created_by, updated_at, updated_by)
                    VALUES (:title, :slug, :summary, :content, :status, :categoryId, :createdAt, :createdBy, :updatedAt, :updatedBy)
                    """;
