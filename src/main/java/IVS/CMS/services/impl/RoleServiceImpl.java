...
// this.roleRepository.updateRolePermissions(savedRole.getRoleId(),
...
// .orElseThrow(() -> new ResourceNotFoundException("Role với id " + id + "
// không tồn tại"));

// if (Boolean.TRUE.equals(currentRole.getIsSystem())) {
// throw new BadRequestException("Không thể chỉnh sửa thông tin role hệ thống: "
...
// if (!currentRole.getRoleName().equalsIgnoreCase(newName) &&
// this.roleRepository.existsByName(newName)) {
// throw new BadRequestException("Role với tên " + req.getName() + " đã tồn
// tại");
// }
...
// this.roleRepository.updateRolePermissions(currentRole.getRoleId(),
// req.getPermissionIds());
...
// throw new BadRequestException("Không thể thêm thủ công vào nhóm
// SUPER_ADMIN");
// }
...
// !ROLE_NORMAL_USER.equalsIgnoreCase(user.getRole().getRoleName())) {
// throw new BadRequestException("Chỉ được chuyển user từ role NORMAL_USER sang
// nhóm mới");
// }
...
// throw new BadRequestException("Không thể gỡ người dùng khỏi nhóm mặc định
// hoặc SUPER_ADMIN");
// }

// Role normalUserRole = this.roleRepository.findByName(ROLE_NORMAL_USER)
// .orElseThrow(() -> new ResourceNotFoundException("Role NORMAL_USER không tồn
// tại"));
